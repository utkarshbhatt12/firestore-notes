...
noteViewButton.addEventListener('click', () => {
  ...
  notesRef.get()
    .then((snapShot) => {
      ...
      // we subtract 2 from it because of the top 'hr' and the
      // bottom 'p' elements then we divide it by 5 because the number
      // of elements being created to display a single note is 5.
      // The result is the number of notes that we have in display
      let numberOfNotes = (
        displayNotesDiv.getElementsByTagName('*').length - 2
      ) / 5;

      // if the list of notes is already populated, first remove them
      // from the viewport then reconstruct the notes again
      // by reading them from the firestore
      if (numberOfNotes > 1) {
        displayNotesDiv.innerHTML = '';
...
// read the values from sessionStorage and populate the
// input fields appropriately
